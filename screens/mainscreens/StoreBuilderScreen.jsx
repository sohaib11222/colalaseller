...
export default function StoreBuilderScreen() {
  ...
  useEffect(() => {
    ...
    const socialLinksData = Array.isArray(storeApi.social_links) ? storeApi.social_links : [];
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={[""]}>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
          ...
          <ThemedText style={[styles.sectionTitle, { color: C.text, marginTop: 20 }]}>
  ...
  return (
    <View style={{ marginBottom: 20 }}>
      ...
      <TouchableOpacity
        activeOpacity={0.9}
        onPress={() => openModal()}
        style={[styles.addLinkBtn, { backgroundColor: C.card, borderColor: C.line }]}
      >
      ...
      {socialLinks.map((link, index) => (
        <View key={index} style={[styles.socialLinkItem, { backgroundColor: C.card, borderColor: C.line }]}>
          <View style={styles.socialLinkInfo}>
            <Ionicons name={getIconForType(link.type)} size={20} color={C.primary} />
            <View style={{ flex: 1, marginLeft: 12 }}>
              <ThemedText style={[styles.socialLinkType, { color: C.text }]}>
                {socialTypes.find(s => s.key === link.type)?.label || link.type}
              </ThemedText>
              <ThemedText style={[styles.socialLinkUrl, { color: C.sub }]} numberOfLines={1}>
                {link.url}
              </ThemedText>
            </View>
          </View>
...
/* ----- Social Link Modal ----- */
function SocialLinkModal({ visible, C, socialTypes, linkType, setLinkType, linkUrl, setLinkUrl, onSave, onClose, isEditing }) {
  if (!visible) return null;
      ...
      <View style={styles.overlay}>
        ...
        <View style={[styles.sheet, { backgroundColor: C.card }]}>
          ...
          <ScrollView style={{ paddingHorizontal: 16, marginTop: 4 }}>
            ...
            <ThemedText style={[styles.modalLabel, { color: C.text }]}>Platform</ThemedText>
              ...
              {socialTypes.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.typeChip,
                    { 
                      backgroundColor: linkType === type.key ? C.primary : '#F3F4F6',
            ...
            {/* URL Input */}
            <ThemedText style={[styles.modalLabel, { color: C.text }]}>URL</ThemedText>
            <View style={[styles.modalInputWrap, { backgroundColor: C.card, borderColor: C.line }]}>
              <TextInput
                placeholder="https://..."
                value={linkUrl}
