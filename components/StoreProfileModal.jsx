...
const openInGoogleMaps = async (addrLike) => {
  try {
    const a = typeof addrLike === "string" ? { fullAddress: addrLike } : (addrLike || {});
...
const labelWithDepth = (title, depth) => `${"   ".repeat(depth)}${title}`.trimStart();
...
const productHasCategory = (p, categoryId) => {
  ...
  const singleIds = [
    p.category_id, p.categoryId, p.category?.id, p.subcategory_id, p.subCategory?.id,
  ].filter(Boolean).map(Number);
  if (singleIds.includes(id)) return true;
  const many = (p.categories || p.category_tree || []).map((c) => Number(c?.id)).filter(Boolean);
...
const productHasBrand = (p, brandId) => {
  ...
  const single = [p.brand_id, p.brandId, p.brand?.id].filter(Boolean).map(Number);
...
export default function StoreProfileModal({
  ...
  */
  ...
  const brandsList = (brandsRes?.data?.data || brandsRes?.data || []).map((b) => ({
    id: Number(b.id),
  ...
  const BrandModal = () => (
    <Modal visible={brandOpen} animationType="slide" transparent onRequestClose={() => setBrandOpen(false)}>
      <TouchableOpacity style={styles.pickerOverlay} activeOpacity={1} onPress={() => setBrandOpen(false)}>
        <View />
      </TouchableOpacity>
      <View style={styles.pickerSheet}>
        <View style={styles.sheetHandle} />
        ...
        <TouchableOpacity
          style={styles.pickerRow}
          onPress={() => { setSelectedBrandId(null); setBrandLabel("Brand"); setBrandOpen(false); }}
        >
          <ThemedText>All</ThemedText>
        </TouchableOpacity>
  ...
  const LocationModal = () => (
    <Modal visible={locOpen} animationType="slide" transparent onRequestClose={() => setLocOpen(false)}>
      <TouchableOpacity style={styles.pickerOverlay} activeOpacity={1} onPress={() => setLocOpen(false)}>
        <View />
      </TouchableOpacity>
      <View style={styles.pickerSheet}>
        <View style={styles.sheetHandle} />
            ...
            <View style={{ marginHorizontal: 16, marginTop: 10 }}>
              ...
              <View style={styles.filtersRow}>
                <TouchableOpacity style={styles.select} onPress={() => setCatOpen(true)}>
                  <ThemedText style={styles.selectLabel}>{categoryLabel}</ThemedText>
                  <Ionicons name="chevron-down" size={16} color="#101318" />
                </TouchableOpacity>
                <TouchableOpacity style={styles.select} onPress={() => setBrandOpen(true)}>
                  <ThemedText style={styles.selectLabel}>{brandLabel}</ThemedText>
                  <Ionicons name="chevron-down" size={16} color="#101318" />
                </TouchableOpacity>
                <TouchableOpacity style={styles.select} onPress={() => setLocOpen(true)}>
                  <ThemedText style={styles.selectLabel}>{locationLabel}</ThemedText>
                  <Ionicons name="chevron-down" size={16} color="#101318" />
                </TouchableOpacity>
              </View>
